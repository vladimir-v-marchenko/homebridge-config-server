    ...
    interfaces[dev].forEach(function(details) {
      if (!details.internal){
        if(details.family == 'IPv4') {
          console.log("Access: https://" + details.address + ":1337");
...
HomebridgeConfig.prototype._onRequest = function(req, res) {
  ...
  switch(requestPath) {
    ...
    case '/css':
      fs.readFile(this.fileDir + '/lib/css/'+requestFile, 'UTF-8', function(err, data) {
        if(err) {
          debug('%s', err);
          res.writeHead(404, {'Content-Type': 'text/plain'});
    ...
    case '/js':
      fs.readFile(this.fileDir + '/lib/js/'+requestFile, 'UTF-8', function(err, data) {
        if(err) {
          debug('%s', err);
          res.writeHead(404, {'Content-Type': 'text/plain'});
